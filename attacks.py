...
def flag_biased(
    ...
    for _ in range(args.m - 1):
        ...
        perturb_data_training = perturb[
            training_idx
        ].detach() + args.step_size * torch.sign(perturb.grad[training_idx].detach())
        perturb.data[training_idx] = perturb_data_training.data

        perturb_data_unlabel = perturb[
            unlabel_idx
        ].detach() + args.amp * args.step_size * torch.sign(
...
def flag_products(
    model, clean, y, adjs, args, optimizer, device, criterion, train_idx=None
):
    model.train()
    ...
    perturb_t = (
        torch.FloatTensor(*clean[: args.batch_size].shape)
        .uniform_(-args.step_size, args.step_size)
        .to(device)
    )
    perturb_un = (
        torch.FloatTensor(*clean[args.batch_size :].shape)
        .uniform_(-args.amp * args.step_size, args.amp * args.step_size)
        .to(device)
    )
    ...
    for _ in range(args.m - 1):
        ...
        perturb_data_t = perturb_t.detach() + args.step_size * torch.sign(
            perturb_t.grad.detach()
        )
        perturb_t.data = perturb_data_t.data
        perturb_t.grad[:] = 0

        perturb_data_un = perturb_un.detach() + args.amp * args.step_size * torch.sign(
            perturb_un.grad.detach()
        )
