...
class GMN(nn.Module):
    ...
    def __init__(
        ...
        for i in range(1, hidden_dim):
            self.ref_points = torch.cat(
                (self.ref_points, nn.init.xavier_uniform_(w, gain=2)), dim=0
            )
        ...
        if prior_centroids is None:
            self.centroids = nn.Parameter(
                ...
                * torch.rand(
                    self.cluster_heads,
                    (self.total_centroids - 1)
                    * (self.hidden_dim // 2 + self.positional_hiddim),
                )
                - 1
            )
        else:
        ...
        self.wm21 = nn.Linear(hidden_dim // 2 + pos_dim, hidden_dim // 2 + pos_dim)
    ...
    def forward(self, x_node, adj, epoch, graph_sizes, c_layer, master_node_flag):
        ...
        if c_layer == 0:
            ...
            # size : same az p
            aranger = (
                torch.arange(adj.shape[1])
                .view(1, 1, -1)
                .repeat(adj.shape[0], self.num_centroids[c_layer], 1)
            )
            # size: same az p
            graph_broad = graph_sizes.view(-1, 1, 1).repeat(
                1, self.num_centroids[c_layer], adj.shape[1]
            )
            if self.has_gpu:
        ...
        if self.master_node_flag:
            new_adj, new_feat, hardening_loss, h_prime = self.query(
                master_feat, master_adj, c_layer
            )
        else:
    ...
    def query(self, x_node, adj, cluster_layer_num):
        if cluster_layer_num == 0:
            x_node = self.leakyrelu(
                F.dropout(self.wm1(x_node), p=self.dropout, training=self.training)
            )
            x_node = self.leakyrelu(
                F.dropout(self.wm2(x_node), p=self.dropout, training=self.training)
            )
            adj_feat = self.leakyrelu(
                F.dropout(self.adjlayer(adj), p=self.dropout, training=self.training)
            )
            h_prime = torch.cat((x_node, adj_feat), 2)
        else:
            h_prime = self.leakyrelu(
                F.dropout(self.wm21(x_node), p=self.dropout, training=self.training)
            )
    ...
    def cluster_block(self, x, adj, cluster_layer_num):
        ...
        batch_centroids = self.centroids[
            :,
            cumsum[cluster_layer_num]
            * (self.hidden_dim // 2 + self.positional_hiddim) : cumsum[
                cluster_layer_num + 1
            ]
            * (self.hidden_dim // 2 + self.positional_hiddim),
        ]
        batch_centroids = torch.unsqueeze(
            batch_centroids.view(
                self.cluster_heads, -1, (self.hidden_dim // 2 + self.positional_hiddim)
            ),
            0,
        ...
        # same size az points
        batch_centroids_broad = torch.unsqueeze(batch_centroids, 3).repeat(
            1, 1, 1, points.shape[3], 1
        )
        if self.has_gpu:
        ...
        if self.mask is not None:
            self.mask_broad = torch.unsqueeze(self.mask, 1).repeat(
                1, self.cluster_heads, 1, 1
            )
        ...
        if self.p2p:
            if self.master_node_flag:
                ...
                new_adj = (
                    torch.unsqueeze(dg, 0)
                    .repeat(new_adj.shape[0], 1, 1)
                    .to(self.device)
                )
