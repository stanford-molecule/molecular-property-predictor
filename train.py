...
# def _train(model, epoch, args, optimizer1, optimizer2, optimizer3, lr, device, total_num_cluster):
#     labels_list, preds_list, loss_list, labels_list_val = [], [], [], []
#     preds_list_val, scores_list, scores_list_val = [], [], []
#
#     model.train()
#     if (epoch + 1) % args.decay_step == 0:
#         optimizer1, optimizer2, optimizer3, lr = adjust_learning_rate(optimizer1, optimizer2, optimizer3, lr, decay=0.5)
#
#     start = time.time()
...
#             # i think `total_num_cluster == 1` is redundant
...
#
#         if (epoch + 1) % args.backward_period == 1 and len(args.num_centroids) > 1 and args.learn_centroid is not 'f':
#             model.centroids.requires_grad_(True)
#             if args.learn_centroid in ('a', 'c'):
#                 hard_loss.backward()
#         else:
...
#
#     if (epoch + 1) % args.backward_period == 1 and len(args.num_centroids) > 1 and args.learn_centroid != 'f':
#         model.centroids.requires_grad_(True)
#
#         if args.avg_grad:
...
def _run_fold(ds, args, k, mask_nodes):
    print('*' * 40)
    ...
    return best_val_acc, best_val_roc
...
def train(ds, args, mask_nodes=True):
    ...
    for k in range(ds.num_folds):
        best_val_acc, best_val_roc = _run_fold(ds, args, k, mask_nodes)
